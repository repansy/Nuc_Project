...
parser.add_argument("--epochs", type=int, default=20, help="Number of training epochs")     # 50
parser.add_argument("--milestone", type=int, default=30, help="When to decay learning rate; should be less than epochs")
parser.add_argument("--lr", type=float, default=1e-3, help="Initial learning rate")
parser.add_argument("--outf", type=str, default="/home/repansy/Nuc_Project/logs", help='path of log files')
...
def DCNN_train():
    ...
    for epoch in range(opt.epochs):
        ...
        psnr_val = 0
        ssim_val = 0
        for i, data in enumerate(loader_value, 0):
            img_val = data
...
def SWIR_train():
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            # training step
            model.train()    # model.zero_grad() 目前不需要
            optimizer.zero_grad()
            img_train = data
            ...
            imgn_train = img_train + noise
            img_train, imgn_train = Variable(img_train.cuda()), Variable(imgn_train.cuda())
            noise = Variable(noise.cuda()) # noise = noise.cuda() # 
            out_train = model(imgn_train)
            loss = criterion(out_train, noise) / (imgn_train.size()[0]*2)
            loss.backward()
            optimizer.step()
